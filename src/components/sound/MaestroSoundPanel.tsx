    ...
    <section className="rounded-xl bg-slate-900 border border-slate-700 p-4">
      <div className="flex items-start justify-between gap-4 mb-3">
        <div>
          ...
          <div className="text-xs text-slate-500 mt-1">Generate a performance master and register it into RenderCache. Mock is built-in; Local Server connects to the Python sound engine.</div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        <div>
          ...
          <select value={engine} onChange={(e) => handleEngineChange(e.target.value)} className="w-full h-9 rounded bg-slate-950 border border-slate-700 px-2 text-xs text-slate-200 outline-none focus:border-blue-500">
            ...
            <option value="local_ai">Local Sound Server</option>
            <option value="external_runtime">External Runtime</option>
          </select>
        </div>
        <div className="col-span-2">
          <div className="text-[10px] text-slate-500 uppercase tracking-wider mb-1">Local Server URL</div>
