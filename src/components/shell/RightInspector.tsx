...
import { NoteInspector } from '@/components/inspector/NoteInspector';
import { PracticeInspector } from '@/components/inspector/PracticeInspector';
import { BackingInspector } from '@/components/inspector/BackingInspector';
import { BuskingInspector } from '@/components/inspector/BuskingInspector';

// ── Inspector Context Resolver ──────────────────
...
function useInspectorContext(): InspectorContext {
  ...
  return useMemo(() => {
    switch (mode) {
      ...
      case 'editor':
      default:
        // Editor mode: note > track > project
...
function InspectorBreadcrumb({ context }: { context: InspectorContext }) {
  ...
  const trackName =
    selectedTrackIndex !== null ? tracks[selectedTrackIndex]?.name : null;

  const crumbs: string[] = [name || 'Untitled'];

  if (context === 'track' || context === 'note') {
    crumbs.push(trackName || `Track ${(selectedTrackIndex ?? 0) + 1}`);
  }
  if (context === 'note') {
    crumbs.push('Note');
  }
  if (['practice', 'backing', 'busking'].includes(context)) {
    crumbs.push(contextLabels[context]);
  }
...
function InspectorContent({ context }: { context: InspectorContext }) {
  switch (context) {
    case 'project':
      return (
        <FeatureGate featureId="inspector_panel" mode="overlay">
          <ProjectInspector />
        </FeatureGate>
      );
    case 'track':
    ...
    case 'practice':
      return (
        <FeatureGate featureId="practice_visual_guide" mode="blur">
          <PracticeInspector />
        </FeatureGate>
      );
    case 'backing':
      return (
        <FeatureGate featureId="ai_band_generator" mode="blur">
          <BackingInspector />
        </FeatureGate>
      );
    case 'busking':
      return (
        <FeatureGate featureId="busking_playback" mode="blur">
          <BuskingInspector />
        </FeatureGate>
      );
    case 'none':
      return (
        <div className="flex-1 flex items-center justify-center text-slate-500 text-sm p-6 text-center">
          No inspector available for this mode.
        </div>
      );
  }
}
  ...
  return (
    <aside
      className="
        ...
        bg-[#0f172a] border-l border-slate-700/60
        text-slate-300 text-[13px]
        select-none overflow-hidden
      "
      aria-label="Right Inspector Panel"
    >
      {/* ── Header ─────────────────────────────── */}
      <div className="flex items-center justify-between h-10 px-3 border-b border-slate-700/60 shrink-0">
        <div className="flex items-center gap-2 text-slate-400">
          <PanelRight size={14} />
          <span className="font-semibold text-[12px] uppercase tracking-wider">
            {contextLabels[context] || 'Inspector'}
          </span>
        </div>
        <button
